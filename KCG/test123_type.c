...
static kcg_int8 const TU_init_V_lustre_time = init_V_lustre_time;
...
static SimConstValUtils const kcg_int8_constants[] = {
    { "lustre_time::init_V", &TU_init_V_lustre_time },
    { "init_V", &TU_init_V_lustre_time },
};
...
int string_to_kcg_int8(const char *str, void *pValue, char **endptr)
{
    ...
    if (nRet == 0 && pConverter->m_pfnStringToConstant) {
        nRet = pConverter->m_pfnStringToConstant(str, pValue, kcg_int8_constants, sizeof(kcg_int8_constants) / sizeof(kcg_int8_constants[0]), sizeof(kcg_int8), endptr);
        if (nRet != 0) {
            return 2;
        }
    }
